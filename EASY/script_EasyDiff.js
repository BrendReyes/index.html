...
function flipCard({target: clickedCard}) {
  ...
  if(clickedCard !== cardOne && !disableDeck && timeLeft > 0) {
      ...
      let cardTwoNum = cardTwo.querySelector(".back-view h1").innerText;
...
function matchCards(num1, num2) {
  if(num1 === num2) { //if the 2 cards matched
      ...
      if(matchedCard == 10 && timeLeft > 0) {//if the user has matched all the cards 
...
function shuffleCard() {
  ...
  for(var i = 0; i < 10; i++){ //this is were randomly generating numbers for math calculation
